...
- `GET /shows/by-imdb/{id}` can 404 for a show genuinely in the local catalogue if nobody has visited it by key yet AND TVMaze's own `/lookup/shows?imdb=` index (measurably less complete than TVMaze's *general* search — confirmed for several well-known shows, #474) doesn't have it. `epguides_by_imdb_bridge_miss_total` (added #474) is the volume signal for this gap; a bulk reverse-index build (paginating TVMaze's full show database, or an equivalent external dataset) was scoped and deliberately deferred pending that counter showing recurring volume rather than a long-tail handful of shows — see #474 for the two competing designs considered.
