...
These rules bind **every** agent working in this repo — Claude, Codex, OpenCode, Muse — regardless of tool. They are the shared contract; your tool-specific file (`CLAUDE.md` / `AGENTS.md` / your config) adds only tool mechanics on top. This block is machine-synced — do not edit it inline.
...
- **Leave every worktree RECOVERABLE-FROM-REMOTE before you stop — for ANY reason** (task done, context exhausted, timeout, error, hand-off). Before your last turn, your work must be in one of these states, and you must say which:
  - **pushed + PR open** → the normal done state; remove the worktree after the PR merges.
  - **pushed, no PR yet** → say so explicitly and name the branch: nothing tracks the work, and it is invisible to every `gh pr list` and backlog query.
  - **nothing pushed** → push it (a draft PR is fine) **or** state in one line that you are discarding it and why. Never leave uncommitted or unpushed work as your final state.
  - **throwaway** (a worktree you made for a quick check) → remove it in the same breath you finish the check.
- **Removing someone else's worktree: check state first, never blanket-clean.** Safe to remove only when the work is recoverable from the remote — its PR is merged, **or** the branch is pushed with no unpushed commits and a clean tree. Verify with `git -C <wt> status --porcelain` (must be empty) and `git -C <wt> log --oneline origin/<branch>..HEAD` (must be empty). A worktree holding the only copy of something is *never* debris, however old it looks.
- **Cross-clone edit leak (most damaging):** Only Edit/Write/`sed -i` paths **inside your worktree**. Verify with `git -C <worktree> status` — your edits must appear there, never in the main clone. Shell writes can bypass native edit hooks; this rule binds every harness regardless of its native hook coverage.
...
- **`refs/stash` is shared across every worktree of a clone, not scoped to yours.** Never `git stash` in an agent worktree. Save/restore via a scratch diff instead: `git diff > /tmp/<slug>.patch` → `git apply`.
- **A worktree forks secret state stored inside the repo.** The `refs/stash` lesson inverted: there one ref was shared across worktrees that each expected privacy; here genuinely global secret state lives as a per-clone file each worktree silently forks, frozen at its base commit. Never write shared secret state from a worktree — land rotations via PR, never uncommitted; never `git checkout --`/`stash`/`reset` a dirty secret file (that destroys an in-flight rotation with no recovery); never `--ours`/`--theirs` an encrypted blob (that silently deletes the other side's keys) — rebuild from `origin/main` plus your own keys.
- **git records when a commit landed, never who wrote it.** Provenance doubt alone is never grounds to revert, reset, or force-push; verify with diff/log, not memory.
...
- **Deploy from a clean on-main clone, never from a worktree.** A detached-HEAD, dirty, or behind-main tree may be rejected by the deploy-currency guard.
...
- **Resuming an aged worktree (hand-off / idle / post-crash reattach):** Run `git -C <worktree> fetch origin` then `git rebase origin/main` before continuing. If the rebase is non-trivial or the branch is badly diverged, open a fresh worktree off `origin/main` and cherry-pick instead.

- **Preserve task continuity across compaction and handoff.** Keep the objective, latest user constraints, decisions, verified progress and next action in the existing issue/PR task record at meaningful milestones. After compaction or resume, reconcile the summary with that record and current Git/PR state before continuing; never repeat a completed action solely because it vanished from context. Preserve identity on resume; record adoption for a replacement session. Use the shared harness session-recovery procedure for checkpoint and handoff details.

### Multi-agent coordination (several agents run concurrently as the same GitHub user)
- **Establish a session identity before creating a branch or writing to GitHub (every harness, including Codex, OpenCode, and Muse).** Recover this session's identity from its startup context or persisted record, or allocate one via `bash ~/.dotfiles/agent-harness/hooks/agent-identity.sh <harness> [session-key]`; announce it, use it as the branch prefix, and stamp it on issue/PR bodies (`**Agent**: <identity> (<harness>)`) and claim, progress, review and handoff comments. Identity is provenance, never proof of liveness or permission to take another agent's work.
...
- **Branch names are `<agent>/<kebab-slug>` and contain a `/` — never embed a raw branch name in a file path, container name, or metric label.** Sanitize first (`${branch##*/}`, or `tr '/' '-'`), the same class of hazard as the worktree-path rules above. Cross-ref the scratchpad unique-naming rule (issue number + a literal token, never a raw branch name).
...
- **Direct host/guest power commands require explicit operator approval for the specific target and action.** The shared shell guard blocks literal reboot, shutdown and guest stop commands, including common shell/SSH wrappers. After approval, prefix the tool command with `AGENT_POWER_OVERRIDE='operator approved <target/action and reason>'` (at least 20 characters); never invent approval or proceed after an instruction to wait. General rollout permission, an incident override and inherited environment settings do not authorize power actions. This is an accidental-action tripwire, not a security sandbox or proof of human approval; sanctioned management commands retain their own safeguards and human-terminal break-glass remains available.
...
- **Rotate a credential on real exposure — not by rule, and not by age.** Ask two questions and write the answers on the issue: *who can newly read it*, and *what does it newly let them do?* **Rotate** when the value left your control (a public repo, an external service, third-party logs, a chat/issue/PR), when a **broad** credential (write, admin, or many repos) became readable by a principal who did not already hold equivalent access, or on evidence of misuse. **Do not create maintainer work** for a narrow read-only credential seen only by the operator, root, or accounts that already hold `sudo` there; for a value inside the operator's own `0700` home or transcripts; or for **age alone** when the only mint path is a human in a vendor UI — record that key as accepted with its mint-path evidence, compensating control and revocation path instead. If you cannot establish who could have read it, treat it as having left your control and rotate. When a rotation is agent-doable end to end, it costs nothing: just do it.
- **Scratchpad names must be unique from the moment of creation — and re-readable later.** A session's scratch directory is shared by every agent in that session, so two agents choosing the same obvious name silently overwrite each other — no error, the loser's bytes are simply gone. Bake the issue number *and* a short literal token you choose once into any file you will read back (`scratchpad/pr-body-1234-k3f.md`, never `pr_body.md`). **Do not use `$$` for this.** Each shell tool call runs in a NEW shell, so `$$` differs between the call that writes the file and the call that reads it. If you have lost the token, recover by globbing (`ls scratchpad/pr-body-1234-*.md`), never by regenerating `$$`. After publishing via `--body-file`, **re-fetch the live object** and confirm it matches.
...
- **Wait for review, then disposition it.** When a PR has a configured or requested human or automated review, do not merge while that review is pending. After the final push, read every review and review comment against the exact head. For every finding, either fix it and rerun the affected checks, or record an evidence-backed rationale for no change; record that disposition before merge, **on the surface the finding lives on** — an inline review-thread finding is answered in that thread and the thread resolved, a top-level finding with a top-level reply. When the environment provides a machine-readable disposition helper, use it rather than prose alone. A `COMMENTED` review is not an empty review, and green CI is not a substitute for reading it. If the only review predates the final head, revalidate its findings against the final diff and say so in the PR.
...
- **Never merge red.** A failing/missing required check or a change-requesting review is a signal to FIX, not to merge.
...
- **Ad-hoc Python via a shell tool:** do NOT backslash-escape quotes inside a heredoc f-string (`peak[\"run\"]` → `SyntaxError: unexpected character after line continuation character`). Prefer (a) writing the script to a file and running it, (b) single-quoted dict keys inside a double-quoted f-string (`f"{d['k']}"`), or (c) `%`/`.format()`. **Never put backticks or `$(...)` inside a double-quoted `python3 -c "…"`** — zsh command-substitutes them *before* Python sees the string, silently splicing command output into your source, which then commits cleanly and passes lint. The same risk applies to an **unquoted** heredoc (`<<EOF`): the shell interpolates backticks/`$(...)`/`$VAR` inside it exactly like a double-quoted string before Python ever sees the source. Prose containing shell metacharacters must go through a **quoted** heredoc (`<<'PYEOF'`, single-quoted delimiter) or a file, never `-c "…"` or an unquoted heredoc.
- **Never judge repo syntax with the host `python3`.** It can be an older version than the repo's own pinned `requires-python`; PEP 758 (3.14) re-allows un-parenthesized `except A, B:`, which an older host interpreter rejects with `SyntaxError: multiple exception types must be parenthesized` on code that is correct for the repo's actual runtime. Parse or `py_compile` with `uv run python` (or the repo's own test runner/Docker image), never bare `python3`. If a "syntax error" appears on a file that is on `main` in a repo whose CI is green, that contradiction is the tell — suspect the interpreter before the code.
...
- **`priority:` is the ONLY ordering signal.** `priority:p0..p3` decides what to work on next. `severity:` describes **impact if the thing occurs** and must never be read as urgency. Set priority/effort **once, as labels**; the board's Priority/Effort fields are derived views — never hand-edit them.
...
- **A trailing pipe discards the exit status you care about.** `cmd | tail`, `cmd | grep …` report the LAST command's status, so a reported "exit code 0" on a piped invocation is evidence of nothing. Never conclude a suite, build, push or commit succeeded from a piped run — run it unpiped and redirect to a file, or capture `${pipestatus[1]}` (zsh) / `${PIPESTATUS[0]}` (bash), or grep the output for the real verdict line.
- Pre-commit and pre-push run automatically; **never** `--no-verify`. Fix the failure instead.
- The coverage floor is a fixed **95%** — never lower a gate to pass, and never RAISE it above 95 either.
- **A prober being down is not the probed thing being down.** An alert derived from a probe must distinguish "the target failed" from "the check could not run". Emit an explicit unknown/stale state and alert on it separately; "cannot verify" is never "verified broken".
...
- `GET /shows/by-imdb/{id}` resolves a cold ID through TVMaze's `/lookup/shows?imdb=` (which answers with a 301 that must be followed) and bridges to the catalogue by the master list's `TVmaze` id column. It still 404s when TVMaze has no record for the ID, when the show is not in the epguides catalogue (no row with that TVMaze id and no unique exact-title match among rows without one), when the bridge is ambiguous (a TVMaze id shared by several catalogue shows with no unique title/year match), or when the master list is unavailable. `epguides_by_imdb_bridge_miss_total` counts those misses; the log line carries the reason (#474).
